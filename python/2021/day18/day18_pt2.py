...
numbers = []
for line in f:
    numbers.append(split_input(line))
...
for n in first_numbers:
    for m in second_numbers:
        print(n, m)
        if n == m:
            continue
        first_value = n.copy()
        second_value = m.copy()
        output = add_numbers(first_value, second_value)
        if magnitude(output) > largest:
            largest = magnitude(output)
        first_value = n.copy()
        second_value = m.copy()
        output = add_numbers(second_value, first_value)
        if magnitude(output) > largest:
            largest = magnitude(output)
